...
func getOsToolsSubdir() (string, error) {
	platform, set := os.LookupEnv("SEMMLE_PLATFORM")
	if set && platform != "" {
		return platform, nil
	}
...
func getExtractorDir() (string, error) {
	extractorRoot := os.Getenv("SEMMLE_EXTRACTOR_GO_ROOT")
	if extractorRoot == "" {
		log.Print("SEMMLE_EXTRACTOR_GO_ROOT not set.\nThis binary should not be run manually; instead, use the Semmle CLI or VSCode extension. See https://securitylab.github.com/tools/semmle.\n")
		...
		if err == nil {
			...
			return "", errors.New("SEMMLE_EXTRACTOR_GO_ROOT not set, and could not determine path of this executable: " + err.Error())
		}
	}
