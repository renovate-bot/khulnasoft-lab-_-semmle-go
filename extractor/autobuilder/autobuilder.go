...
// to the Semmle source directory as well as whether the build command executed successfully.
...
// checkExtractorRun checks whether the Semmle Go extractor has run, by checking if the source
// archive directory is empty or not.
func checkExtractorRun() bool {
	srcDir := os.Getenv("SEMMLE_EXTRACTOR_GO_SOURCE_ARCHIVE_DIR")
	if srcDir != "" {
