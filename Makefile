...
ifeq ($(OS),Windows_NT)
EXE = .exe
SEMMLE_PLATFORM = win64
else
EXE =
UNAME_S := $(shell uname -s)
ifeq ($(UNAME_S),Linux)
SEMMLE_PLATFORM = linux64
endif
ifeq ($(UNAME_S),Darwin)
SEMMLE_PLATFORM = osx64
endif
endif

SEMMLE_TOOLS = $(addprefix semmle-tools/,autobuild.cmd autobuild.sh pre-finalize.cmd pre-finalize.sh index.cmd index.sh linux64 osx64 win64 tracing-config.lua)

EXTRACTOR_PACK_OUT = build/semmle-extractor-go

BINARIES = go-extractor go-tokenizer go-autobuilder go-build-runner go-bootstrap go-gen-dbscheme

.PHONY: tools tools-semmle tools-semmle-full clean autoformat \
...
tools-semmle: tools-$(SEMMLE_PLATFORM)

tools-semmle-full: tools-linux64 tools-osx64 tools-win64
...
extractor-common: semmle-extractor.yml LICENSE ql/lib/go.dbscheme \
	tools/tokenizer.jar $(SEMMLE_TOOLS)
	rm -rf $(EXTRACTOR_PACK_OUT)
	mkdir -p $(EXTRACTOR_PACK_OUT)
	cp semmle-extractor.yml LICENSE ql/lib/go.dbscheme ql/lib/go.dbscheme.stats $(EXTRACTOR_PACK_OUT)
	mkdir $(EXTRACTOR_PACK_OUT)/tools
	cp -r tools/tokenizer.jar $(SEMMLE_TOOLS) $(EXTRACTOR_PACK_OUT)/tools

extractor: extractor-common tools-semmle
	cp -r tools/$(SEMMLE_PLATFORM) $(EXTRACTOR_PACK_OUT)/tools

extractor-full: extractor-common tools-semmle-full
...
ql/lib/go.dbscheme: tools/$(SEMMLE_PLATFORM)/go-gen-dbscheme$(EXE)
	$< $@
...
ql/lib/go.dbscheme.stats: ql/lib/go.dbscheme build/stats/src.stamp extractor
	...
	semmle database create -l go -s build/stats/src -j4 --search-path . build/stats/database
	semmle dataset measure -o $@ build/stats/database/db-go

test: all build/testdb/check-upgrade-path
	semmle test run ql/test --search-path . --consistency-queries ql/test/consistency
  #	use GOOS=linux because GOOS=darwin GOARCH=386 is no longer supported
	env GOOS=linux GOARCH=386 semmle$(EXE) test run ql/test/query-tests/Security/CWE-681 --search-path . --consistency-queries ql/test/consistency
	cd extractor; go test -mod=vendor ./... | grep -vF "[no test files]"
	bash extractor-smoke-test/test.sh || (echo "Extractor smoke test FAILED"; exit 1)

.PHONY: build/testdb/check-upgrade-path
build/testdb/check-upgrade-path : build/testdb/go.dbscheme ql/lib/go.dbscheme
	semmle dataset upgrade build/testdb --search-path ql/lib
...
build/testdb/go.dbscheme: ql/lib/upgrades/initial/go.dbscheme
	...
	semmle dataset import -S ql/lib/upgrades/initial/go.dbscheme build/testdb build/empty.trap
...
sync-dataflow-libraries:
	...
	do\
		curl -s -o ./ql/lib/semmle/go/dataflow/internal/$$f https://raw.githubusercontent.com/khulasoft-lab/semmle/$(DATAFLOW_BRANCH)/java/ql/lib/semmle/code/java/dataflow/internal/$$f;\
	done
